  ...
  try {
    ...
    if (randomWordObj) {
      ...
      return res
        ...
        .json({ ...httpStatusCodes.HTTP_201_CREATED, wordsIds, randomWordObj });
    }

    return res
      .status(httpStatusCodes.HTTP_409_CONFLICT.code)
      .json({
        ...httpStatusCodes.HTTP_409_CONFLICT,
        error: "No more words available in this level or theme combination",
      });
  } catch (error: any) {
    res
